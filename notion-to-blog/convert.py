...
环境变量:
    NOTION_API_TOKEN   Notion API token（或 ~/.config/notion/api_key）
    BLOG_ROOT          Jekyll 博客根目录（必须，不含 _config.yml 则报错）
"""
...
BLOG_ROOT = Path(os.environ.get("BLOG_ROOT", ""))
if not BLOG_ROOT.is_dir():
    raise SystemExit("❌ BLOG_ROOT 未设置或不是有效目录")
if not (BLOG_ROOT / "_config.yml").exists():
    raise SystemExit(f"❌ {BLOG_ROOT}/_config.yml 不存在，BLOG_ROOT 指向的不是 Jekyll 博客根目录")
